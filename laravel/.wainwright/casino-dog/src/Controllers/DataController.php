...
class DataController
{
    ...
    public static function get_demolink($gid)
    {
        ...
        if(config('casino-dog.wainwright_proxy.get_demolink') === 1) {
            $prepend_cors_anywhere_proxy = config('casino-dog.cors_anywhere');
            $get = self::route_get_demolink($prepend_cors_anywhere_proxy.$url, $select['source_schema']);
        } else {
        ...
        if($source_schema === 'parimatch') {
            ...
            if(config('casino-dog.wainwright_proxy.get_demolink') === 1) {
                $prepend_cors_anywhere_proxy = config('casino-dog.cors_anywhere');
                $retrieve = $kernel->parimatch_curl($prepend_cors_anywhere_proxy.'https://pari-match.com/service-discovery/service/pm-casino/api/eva/slots/lobby/game/demo/'.$gid.'?lobbyUrl=https%3A%2F%2Fpari-match.com%2Fen%2Fcasino%2Fslots');
            } else {
    ...
    public function gameslist_batch($batch_id)
    {
        ...
        if($select_job->schema === 'parimatch') {
            ...
            if(config('casino-dog.wainwright_proxy.get_gamelist') === 1) {
                $prepend_cors_anywhere_proxy = config('casino-dog.cors_anywhere');
                $http_get = $this->parimatch_curl($prepend_cors_anywhere_proxy.$select_job->link);
            } else {
            ...
            if(config('casino-dog.wainwright_proxy.get_gamelist') === 1) {
                $prepend_cors_anywhere_proxy = config('casino-dog.cors_anywhere');
                $http_get = $this->softswiss_curl($prepend_cors_anywhere_proxy.$select_job->link);
            } else {
    ...
    public function gameslist_process_parimatch($game_options, $data) {
        ...
        if($filterkey !== NULL && $filtervalue !== NULL) {
            if($prepareArray[$filterkey] === $filtervalue) {
                ...
                $kernel = new \Wainwright\CasinoDog\Models\RawGameslist;
                $dispatch = $kernel->start_job($gameID);
            }
        } else {
            RawGameslist::insert($prepareArray);
            $kernel = new \Wainwright\CasinoDog\Models\RawGameslist;
